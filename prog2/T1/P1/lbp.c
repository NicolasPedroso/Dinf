...
void calculaMediana(imagemPGM *img, unsigned char **imagemLBP) {
    ...
    for (int i = 1; i < img->altura - 1; i++) {
        for (int j = 1; j < img->largura -1; j++) {
            unsigned char valorMediana = 0;

            vetor[0] = img->pixels[i-1][j-1];
            vetor[1] = img->pixels[i-1][j];
            vetor[2] = img->pixels[i-1][j+1];
            vetor[3] = img->pixels[i][j+1];
            vetor[4] = img->pixels[i+1][j+1];
            vetor[5] = img->pixels[i+1][j];
            vetor[6] = img->pixels[i+1][j-1];
            vetor[7] = img->pixels[i][j-1];
            vetor[8] = img->pixels[i][j];

            /*Ordena aqui*/

            qsort(&vetor, 9, sizeof(int), compara);

            valorMediana = vetor[5];

            imagemLBP[i-1][j-1] = valorMediana;
        }
    }
