...
void adiciona_aresta(int id, int u_id, int v_id, grafo G) {
    ...
    novo_aresta->u = u;
    novo_aresta->v = v;
    novo_aresta->peso = grau(u) + grau(v); 

    empilha(novo_aresta, arestas(G));

    empilha(novo_aresta, u->fronteira);
    empilha(novo_aresta, v->fronteira);

    /*Atualiza as arestas dos vertice u e v*/
    for (no n = primeiro_no(fronteira(u)); n != NULL; n = proximo(n)) {
        aresta a = conteudo(n); 
        a->peso = grau(vertice_u(a)) + grau(vertice_v(a));
    }

    for (no n = primeiro_no(fronteira(v)); n != NULL; n = proximo(n)) {
        aresta a = conteudo(n); 
        a->peso = grau(vertice_u(a)) + grau(vertice_v(a));
    }
